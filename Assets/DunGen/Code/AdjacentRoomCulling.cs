...
namespace DunGen
{
	...
	public class AdjacentRoomCulling : MonoBehaviour
	{
		...
		protected virtual void LateUpdate()
		{
			...
			else if (!currentTile.Bounds.Contains(transform.position))
		...
		private Tile FindCurrentTile()
		{
			...
			{
				if (tile.Bounds.Contains(transform.position))
					return tile;
			}
		...
		private Tile SearchForNewCurrentTile()
		{
			...
			foreach (var door in currentTile.Placement.UsedDoorways)
			{
			...
			while (tilesToSearch.Count > 0)
			{
				...
				else
				{
					...
					foreach (var door in tile.Placement.UsedDoorways)
					{
						var adjacentTile = door.ConnectedDoorway.Tile;
