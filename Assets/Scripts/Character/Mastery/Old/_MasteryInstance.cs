...
public class _MasteryInstance
{
    public MasteryGroup MasteryGroup { get; private set; }
    public _Mastery Mastery { get; private set; }

    public int CurrentLevel { get; set; } = 0;

    public _MasteryInstance(_Mastery mastery, MasteryGroup masteryGroup)
    {
        Mastery = mastery;
    ...
    public void LevelUp()
    {
        ...
        {
            Debug.LogWarning(
                $"{Mastery.MasteryName} não pode subir de nível. Atual: {CurrentLevel}, Máximo: {Mastery.MasteryMaxLevel}");
        }
        else if (Mastery.MPCost > MasteryGroup.Character.CurrentMp)
        {
            Debug.LogWarning(
                $"{MasteryGroup.Character.Base.CharacterName} não possui MP suficiente para subir de nível.");
        }
        else
        {
